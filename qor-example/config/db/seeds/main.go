		...
		for _, cv := range p.ColorVariations {
			...
			for _, i := range cv.Images {
				...
				if err := DraftDB.Create(&image).Error; err != nil {
					log.Println("create color_variation_image (%v) failure, got err %v when %v", image, err, i.URL)
				} else {
...
func createWidgets() {
	...
	topBannerSetting.Name = "home_page_banner"
	...
	if file, err := openFileByURL("http://pic1.zhimg.com/50/v2-af12f7b6f7eb2f2f5f7e2f3b7880cf01_hd.jpg"); err == nil {
		defer file.Close()
	...
	if err := DraftDB.Create(&topBannerSetting).Error; err != nil {
		log.Println("Save widget (%v) failure, got err %v", topBannerSetting, err)
	}
	...
	slideshowSetting.Name = "home_page_banner"
